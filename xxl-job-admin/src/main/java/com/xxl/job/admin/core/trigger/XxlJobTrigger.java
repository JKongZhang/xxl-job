...
public class XxlJobTrigger {
    ...
    /**
     * trigger job
     * 这个方法里会取出任务信息和执行器信息，处理分片参数，
     * 如果任务的路由策略为分片广播，则遍历执行器集群地址，对每个执行器进行任务调度
     *
     * @param jobId                 任务ID
     ...
     * @param executorShardingParam 执行切分参数
    ...
    private static void processTrigger(XxlJobGroup group, XxlJobInfo jobInfo, int finalFailRetryCount,
        ...
        if (group.getRegistryList() != null && !group.getRegistryList().isEmpty()) {
            if (ExecutorRouteStrategyEnum.SHARDING_BROADCAST == executorRouteStrategyEnum) {
                ...
                // 获取执行器路由地址信息
                // todo 重要：此处使用了策略模式，根据不同的策略使用不同的实现类，
                //  从而选举出本次使用执行器集群地址列表中不同的地址信息，这是xxl-job实现任务路由的地方
        ...
        // 4、trigger remote executor 触发远程执行器执行任务（执行器中的handler）
        ReturnT<String> triggerResult = null;
        if (address != null) {
            // todo 重点：启动执行器， 向执行器发送指令都是从这个方法中执行的
        ...
        // 6、save log trigger-info 更新执行日志
    ...
    public static ReturnT<String> runExecutor(TriggerParam triggerParam, String address) {
        ...
        try {
            //todo 重点：获取ExecutorBiz代理对象，先从缓存（内存map）中取，取不到new一个XxlRpcReferenceBean放进缓存map
            ExecutorBiz executorBiz = XxlJobScheduler.getExecutorBiz(address);
            // 这个run方法不会最终执行，仅仅只是为了触发代理对象的invoke方法，同时将目标的类型传送给服务端，
